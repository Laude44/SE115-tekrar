...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("New lenght of the array is :"+arrayExpander(studentID, 42).length); //Prints the new length of the array

        //Bonus problem
        int[][] scores = new int[6][5]; // changed to 6x5 to show header
        int highestScore = -1; // start with -1 to handle positive randoms
        int[] highestScoreIndex = new int[2];
        highestScoreIndex[0]=0;
        highestScoreIndex[1]=0;
        ...
        for(int i=1;i<6;i++) {//Time to calculate the avarage notes of the students
        int avarageScore=(scores[i][0]+scores[i][1]+scores[i][2]+scores[i][3])/4;
            System.out.println("Avarage score of the student number "+i+" is : "+avarageScore);
        }

        System.out.println("The highest score is : " + highestScore+" and it is the student number "+highestScoreIndex[0]+". Quiz number is "+highestScoreIndex[1]);
    }
